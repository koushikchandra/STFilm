...
metadata alone (\textsc{meta}) matches \textsc{none}, and the morphology MoE regresses to baseline.
...
best result, while organ-id metadata alone (\textsc{meta}) and the morphology MoE do not help. See
Sec.~\ref{sec:exp}.}
\label{tab:main}
\end{table}
...
of magnitude smaller than HisToGene ($149$M). \textsc{meta} adds ${<}\,1.2$k parameters; only the
unhelpful \textsc{moe} inflates the model ($3.0$M). Our best configuration scales the backbone to
