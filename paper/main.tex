...
\begin{abstract}
Predicting spatial gene expression from H\&E histology promises a low-cost surrogate for
spatial transcriptomics (ST), but state-of-the-art models are trained and evaluated
\emph{per cohort} and it is unknown whether a single model generalizes \emph{across organs}.
We study cross-organ generalization for STFlow, a recent whole-slide flow-matching model, and
introduce \textbf{STFiLM}, a family of feature-wise linear modulation (FiLM / adaLN-Zero)
conditioners that adapt the denoiser to each slide while \emph{strictly preserving} STFlow's
SE(2)-equivariant frame-averaging backbone. We evaluate on a leakage-safe cross-organ
re-partition of HEST-1k under two regimes---leave-one-organ-out (LOOO, transfer) and pooled
leave-patient-out (POOLED)---with three seeds, against five accepted baselines, and replicate the
conditioning effect on a second dataset (STImage-1K4M). Conditioning on an inference-available
histology descriptor significantly improves over the unconditioned model
($+0.011$ to $+0.015$ Pearson, paired $t$-test $p\!\approx\!0.02$ in both regimes), a gain
comparable in magnitude to STFlow's own core components. A per-spot local descriptor
(\textsc{local}) attains the best mean and improves \emph{every} one of eleven clinically
relevant biomarkers (11/11, e.g.\ Ki-67, CD8A, KIT), while categorical organ metadata alone and
an elaborate morphology mixture-of-experts do \emph{not} help---the former because unseen
organs fall back to a null token, the latter because added capacity washes out the gain on
small folds. We further show that near-zero LOOO scores on three organs are a \emph{metric artifact}:
the leakage-safe gene panel is near-silent in the held-out organ, and a detection-filtered
re-scoring lifts all methods by ${\sim}0.03$ while preserving the ranking. Our results give a
practical recipe---condition on continuous, inference-available morphology; keep equivariance
intact---for organ-generalizable ST prediction. Code and configs will be released.
\end{abstract}
...
a large body of work that \emph{predicts} ST directly from H\&E~\cite{stnet,histogene,hist2st,triplex},
...
mixture of morphology experts? To answer them we (i) build a leakage-safe cross-organ
...
histology descriptors help ($p\!\approx\!0.02$), whereas organ-id metadata alone and a
morphology mixture-of-experts do not, with an analysis of \emph{why}.
...
Hist2ST~\cite{hist2st} couples a transformer with a kNN graph over spot coordinates; and TRIPLEX~\cite{triplex} fuses spot-, neighborhood-, and slide-level resolutions.
...
with classifier-free guidance~\cite{cfg} for controllable conditioning strength; mixture-of-experts
layers~\cite{switch} route tokens to specialized sub-networks. Injecting such conditioning into a
...
\item \textsc{moe}: per-spot routing over $E$ morphology-expert MLPs on the scalar stream, with
a Switch-Transformer load-balance loss and a spatial-smoothness penalty on the routing.
\end{itemize}
...
so comparisons isolate modeling, not the encoder. We compare against \emph{five} accepted methods,
all feature-matched to the same UNI inputs, splits, panels, and metric: ST-Net~\cite{stnet},
HisToGene~\cite{histogene}, Hist2ST~\cite{hist2st}, BLEEP~\cite{bleep}, and TRIPLEX~\cite{triplex};
...
Table~\ref{tab:main} reports both regimes against five accepted baselines
(ST-Net~\cite{stnet}, HisToGene~\cite{histogene}, Hist2ST~\cite{hist2st}, BLEEP~\cite{bleep},
TRIPLEX~\cite{triplex}), all feature-matched to the same UNI features, splits, panels, and metric.
Conditioning on a histology descriptor improves over unconditioned STFlow in both regimes;
\textsc{local} attains the best mean (LOOO $0.4624$, POOLED $0.7950$). TRIPLEX is the strongest
prior method and a near-peer on LOOO ($0.4623$); \textsc{local} still leads on both regimes and,
as we show below, improves \emph{every} biomarker over its own base. Categorical metadata alone
(\textsc{meta}) matches \textsc{none}, and the morphology MoE regresses to baseline.
...
HisToGene~\cite{histogene} & 2021 & 0.4379 & 0.7766 \\
Hist2ST~\cite{hist2st}     & 2022 & 0.4087 & 0.7689 \\
BLEEP~\cite{bleep}         & 2023 & 0.4231 & 0.7708 \\
TRIPLEX~\cite{triplex}     & 2024 & \underline{0.4623} & \underline{0.7909} \\
\midrule
...
\;+\,\textsc{moe}             &      & 0.4488 & 0.7862 \\
\;+\,\textsc{local} (ours, best) &  & \textbf{0.4624} & \textbf{0.7950} \\
\bottomrule
\end{tabular}
...
features, splits, panels, and metric). \textsc{local} is best overall; TRIPLEX is a strong
near-peer on LOOO, while organ-id metadata alone (\textsc{meta}) and the morphology MoE do not
help. See Sec.~\ref{sec:exp}.}
\label{tab:main}
\end{table}
...
generalization. On this benchmark, however, TRIPLEX is the strongest method overall
(LOOO $0.144$, POOLED $0.740$), ahead of STFiLM: STImage ships heavily downsized whole slides
(${\sim}2000$\,px, ${\sim}8$\,px spot radius), so absolute transfer scores are low and TRIPLEX's
multi-resolution front-end is comparatively advantaged. Our contribution is the conditioning
...
Hist2ST~\cite{hist2st}     & 2022 & 0.1421 & 0.7368 \\
BLEEP~\cite{bleep}         & 2023 & 0.1006 & 0.6819 \\
TRIPLEX~\cite{triplex}     & 2024 & \textbf{0.1439} & \textbf{0.7398} \\
\midrule
...
regimes; TRIPLEX is strongest overall on this heavily downsized-slide benchmark.}
...
Table~\ref{tab:perorgan} gives the full per-organ breakdown against all five baselines on both
benchmarks. It makes the comparison concrete: TRIPLEX wins several individual organs
(lung, prostate, breast, colorectal) while \textsc{local} wins kidney, pancreas, and skin and
ties the overall mean; no prior method dominates across organs.
...
property of the silent panels rather than of any model. STFiLM-\textsc{local} remains the best of
our variants ($0.495$), with TRIPLEX the strongest baseline ($0.500$).
...
HisToGene~\cite{histogene} & 0.438 & 0.461 \\
Hist2ST~\cite{hist2st}     & 0.409 & 0.444 \\
BLEEP~\cite{bleep}         & 0.423 & 0.452 \\
TRIPLEX~\cite{triplex}     & 0.462 & \underline{0.500} \\
\midrule
STFlow (\textsc{none})     & 0.448 & 0.479 \\
STFlow~$+$~\textsc{moe}    & 0.449 & 0.481 \\
STFiLM (\textsc{desc})     & 0.459 & 0.491 \\
STFiLM (\textsc{local})    & \textbf{0.462} & 0.495 \\
\bottomrule
\end{tabular}
...
\subsection{Analysis}
\label{sec:analysis}
Two negative results are informative. \textsc{meta} (organ id) matches \textsc{none} because in
...
descriptors are and categorical organ id is not. \textsc{moe} regresses because added expert
capacity and untuned auxiliary losses overfit the small HEST folds; the simple per-spot
descriptor (\textsc{local}) is both cheaper and better. Notably, the FiLM gain
...
dominates every organ: TRIPLEX leads on lung, prostate, breast, and colorectal, and STFiLM on
kidney, pancreas, and skin. The overall-mean advantage of \textsc{local} therefore reflects
...
of magnitude smaller than TRIPLEX ($13.8$M) or HisToGene ($149$M). \textsc{meta} adds ${<}\,1.2$k
parameters; only the unhelpful \textsc{moe} inflates the model ($3.0$M).
...
of state of the art on every dataset: on STImage-1K4M, TRIPLEX~\cite{triplex} is the strongest
...
HEST's full-resolution patches. This compresses the gap between methods and appears to favor
TRIPLEX's explicit multi-resolution aggregation over a single frozen-patch encoder; a
higher-resolution re-extraction of STImage is a natural follow-up. The absolute transfer scores are
...
hard constraint of STFlow's aggregator); stronger encoders (Prov-GigaPath~\cite{gigapath},
CONCH~\cite{conch}) and larger panels are untested. The \textsc{local}$>$\textsc{desc} margin is
...
while organ-id metadata and a morphology mixture-of-experts do not. We also showed that
