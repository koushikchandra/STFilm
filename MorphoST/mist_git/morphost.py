...
NOTE: set normalize_dist=True to scale neighbour distances by the slide's median
spacing before the RBF. Raw HEST pixel coordinates give neighbour distances ~1e3,
...
class LocalKNNAttention(nn.Module):
    ...
    def forward(self, x, nbr_idx, rbf, use_distance_bias=True, uniform=False):  # x[N,D], nbr_idx[N,k], rbf[N,k,R]
        N, k = nbr_idx.shape
        vf = self.v(x)[nbr_idx].view(N, k, self.h, self.dh)
        if uniform:
            # Ablation: uniform mean over the k neighbours (no query-key attention, no
            # distance bias) — isolates whether content-based attention weighting beats
            # uniform graph aggregation as used by the binary-adjacency baselines.
            a = x.new_full((N, self.h, k), 1.0 / k)
        else:
            q = self.q(x).view(N, self.h, self.dh)
            kf = self.k(x)[nbr_idx].view(N, k, self.h, self.dh)
...
class MorphoBlock(nn.Module):
    ...
    def forward(self, x, nbr_idx, rbf):
        h = self.ln1(x)
        local_out = self.local_attn(h, nbr_idx, rbf, self.use_distance_bias, self.uniform_local)  # [N, D]
...
class MorphoST(nn.Module):
    ...
    def forward(self, feats: torch.Tensor, coords: torch.Tensor, return_hidden: bool = False) -> torch.Tensor:
        """
        ...
        if self.normalize_dist:
            # Scale distances by the slide's characteristic spot spacing so the RBF
            # bandwidths (gammas in [1e-3, 1e1]) actually resolve them. Without this,
            # raw pixel distances (~1e3) drive exp(-gamma*d^2) to 0 and the distance
            # bias is a no-op. Dividing by a distance-only scalar preserves coordinate
            # invariance (and adds scale invariance).
