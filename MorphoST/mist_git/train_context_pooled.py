...
def train_fold(args, train_df, val_df, test_df, gene_list, fold_dir):
    ...
    model = ContextMIST(feat_dim=feat_dim, dim=args.dim, n_genes=len(gene_list), n_layers=args.n_layers,
                     n_heads=args.n_heads, k=args.k, dropout=args.dropout, num_rbf=args.num_rbf,
                     attn_dropout=args.dropout, use_distance_bias=args.use_distance_bias,
                     **CONFIGS[args.config]).to(args.device)
