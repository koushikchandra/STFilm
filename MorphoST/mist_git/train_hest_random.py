...
from morphost_random import RandomKMIST
...
def evaluate(model, test_rows, args, gene_list, cohort, prediction_path=None):
    ...
    for _, row in test_rows.iterrows():
        ...
        pred = model(feats, coords)   # count models return log1p(expected count)
        y = expr
        if args.loss in ("nb", "zinb"):
            y = torch.log1p(expr)     # target loaded as raw counts -> log1p for comparable metrics
        preds_all.append(pred.cpu().numpy()); y_all.append(y.cpu().numpy())
...
def train_fold(args, train_df, val_df, test_df, gene_list, cohort, fold_dir):
    ...
    common = dict(feat_dim=feat_dim, dim=args.dim, n_genes=len(gene_list), n_layers=args.n_layers,
                  n_heads=args.n_heads, k=args.k, dropout=args.dropout,
                  num_rbf=args.num_rbf, attn_dropout=args.dropout,
    ...
    if args.loss in ("nb", "zinb"):
        ...
        model = RandomKMIST(**common).to(args.device)
    ...
    for ep in range(1, args.epochs + 1):
        ...
        for _, row in train_rows:
            ...
            if args.loss in ("nb", "zinb"):
                params = model(feats, coords, return_params=True)
...
def main():
    ...
    p.add_argument("--normalize_dist", action=argparse.BooleanOptionalAction, default=False,
                   help="scale kNN distances by slide-median before RBF (fixes pixel-scale RBF saturation)")
    p.add_argument("--uniform_local", action=argparse.BooleanOptionalAction, default=False,
                   help="ablation: uniform mean over k neighbours instead of QK attention in the local pathway")
    p.add_argument("--loss", default="mse", choices=["mse", "nb", "zinb"],
                   help="mse (log1p Gaussian, default) | nb | zinb (count likelihood on raw counts)")
    p.add_argument("--coordinate_mode", default="distance", choices=["distance", "absolute"])
    args = p.parse_args()
    if args.loss in ("nb", "zinb"):
        args.normalize_method = "identity"   # NB/ZINB likelihood needs raw counts
