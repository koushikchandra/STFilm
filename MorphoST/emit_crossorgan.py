...
HEST_ORGANS  = [("Breast",["IDC","LYMPH_IDC"]), ("Prostate",["PRAD"]), ("Pancreas",["PAAD"]),
...
BASELINES = [
    ("ST-Net",    "results_corrected_crossorgan_baselines",         "results_corrected_stimage_crossorgan_baselines", "stnet"),
    ("HisToGene", "results_corrected_crossorgan_baselines",         "results_corrected_stimage_crossorgan_baselines", "histogene"),
    ("Hist2ST",   "results_corrected_crossorgan_baselines",         "results_corrected_stimage_crossorgan_baselines", "hist2st"),
    ("TRIPLEX",   "results_corrected_crossorgan_baselines",         "results_corrected_stimage_crossorgan_baselines", "triplex"),
    ("Gene-DML",  "results_corrected_crossorgan_baselines",         "results_corrected_stimage_crossorgan_baselines", "genedml"),
    ("HyperST",   "results_corrected_crossorgan_baselines",         "results_corrected_stimage_crossorgan_baselines", "hyperst"),
    ("MorphoST",  "MorphoST/results_corrected_factorial",           "MorphoST/results_corrected_stimage",            "V3"),
]

# ── helpers ──────────────────────────────────────────────────────────────────

def cseed(dp, regime, tag, fold, fld="pearson_mean"):
    v = []
    for f in glob.glob(os.path.join(ROOT, dp, f"{regime}_{tag}_seed*", f"fold_{fold}_results.json")):
        try: v.append(json.load(open(f))[fld])
        except: pass
    # also accept seed-specific dirs
    for s in SEEDS:
        f = os.path.join(ROOT, dp, f"{regime}_{tag}_seed{s}", f"fold_{fold}_results.json")
        if os.path.isfile(f):
            try: v.append(json.load(open(f))[fld])
            except: pass
...
def sti_looo_organ(dp, tag, organ):
    ps = []
    for s in SEEDS:
...
for oname, cohs in HEST_ORGANS:
    cells = {m: hest_looo_organ(hd, t, cohs) for m, hd, _, t in BASELINES}
...
for organ in STI_ORGANS:
    cells = {m: sti_looo_organ(sd, t, organ) for m, _, sd, t in BASELINES}
    avail = {m: v for m, v in cells.items() if v}
    best = max(avail, key=lambda k: avail[k][0]) if avail else None
...
hest_pool = {m: pooled_stat(hd, t) for m, hd, _, t in BASELINES}
sti_pool  = {m: pooled_stat(sd, t) for m, _, sd, t in BASELINES}
