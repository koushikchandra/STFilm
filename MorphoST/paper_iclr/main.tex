...
\medskip
\noindent We investigate six questions: \textbf{RQ1 (Accuracy)} across panel sizes and selection strategies; \textbf{RQ2 (Multi-organ)} whether a single pooled model predicts across diverse organs at once; \textbf{RQ3 (Transfer)} generalization to an entirely unseen organ under LOOO; \textbf{RQ4 (Design)} for encoder, context streams, and neighborhood definition; \textbf{RQ5 (Structure)} for spatial-domain recovery; and \textbf{RQ6 (Efficiency)} for whole-slide inference.
...
\FloatBarrier
\medskip
\subsection{Results for RQ2: Unified multi-organ prediction}
\label{sec:multiorgan}
A single \morph{}, trained jointly on all organs pooled (leave-slide-out cross-validation; no
per-organ specialisation and no organ labels), predicts spatial expression across the diverse organs
...
Table~\ref{tab:pooled6}), ahead of BLEEP ($0.587$) and Hist2ST ($0.556$); on the full seven-organ HEST
panel it reaches $0.760$, narrowly above BLEEP ($0.758$). BLEEP obtains the lowest absolute error.
Thus one shared model serves many organs at once---without the per-organ training of RQ1---provided
each organ is represented during training.

\begin{keybox}
\textbf{Key Finding (RQ2):} A single pooled \morph{} predicts expression across all six organs
simultaneously (best mean PCC $0.599$; $0.760$ on full HEST), so a separate model per organ is
...
\FloatBarrier
\medskip
\subsection{Results for RQ3: Cross-organ transfer to an unseen organ}
\label{sec:transfer}
Leave-one-organ-out (LOOO) is substantially harder than the pooled setting of RQ2 because the target
...
\begin{keybox}
\textbf{Key Finding (RQ3):} The pooled$\to$LOOO drop---from $0.760$ with the organ seen (RQ2) to
...
\FloatBarrier
\medskip
\subsection{Results for RQ4: What drives accuracy and coordinate invariance?}
\label{sec:design}
...
\begin{keybox}
\textbf{Key Finding (RQ4):} Local distance-biased attention is the primary driver of accuracy---
...
\subsection{Results for RQ5: Spatial structure preservation}
\label{sec:structure}
...
\FloatBarrier
\medskip
\subsection{Results for RQ6: Computational efficiency}
