...
\begin{keybox}
\textbf{Core Idea:} MIST predicts a slide's spatial expression in a single forward pass by fusing
three views of context---local distance-biased neighborhoods, global self-attention, and a slide-level
token---over frozen histology features, made invariant to the spot coordinate frame (translation,
rotation, reflection) so a single model transfers across slides without per-dataset retraining.
\end{keybox}
...
\label{sec:setup}
We evaluate on \textbf{HEST-1k} \cite{jaume2024hest} with frozen UNI+CONCH patch features and
$\log(1+x)$ targets; unless stated otherwise the target is the 50-gene HEST HVG panel. Cross-organ
...
\emph{LOOO} (train on all-but-one organ, test the held-out organ). We report PCC---and MSE/MAE where
noted---over outer folds and three seeds, with checkpoints chosen on inner validation and evaluated
once on the outer fold. Baselines ST-Net \cite{he2020stnet}, Hist2ST \cite{zeng2022hist2st}, BLEEP
...
\label{sec:rq1}
We first establish accuracy on organs the model has \emph{seen}, both when a model is trained
per cohort (intra-cohort) and when a \emph{single} model is trained jointly across organs (pooled).
...
\medskip
Beyond per-organ models, a \emph{single} \morph{} trained jointly on all organs pooled (leave-slide-out
cross-validation; no per-organ specialisation and no organ labels) predicts across the diverse organs
at once, adapting to each slide through its shared local, global, and slide-level context rather than a
dedicated per-tissue model. On the six-organ subset it attains the best mean PCC ($0.599$;
Table~\ref{tab:pooled6}), ahead of BLEEP ($0.587$) and Hist2ST ($0.556$), and reaches $0.760$ on the
full seven-organ HEST panel (BLEEP $0.758$). Thus one shared model serves many organs at once, without
per-organ retraining, provided each organ is represented during training.
...
organ} (intra-cohort PCC@50 $0.470$ vs.\ $0.426$ for BLEEP, winning all seven cohorts) and as a
...
\label{sec:transfer}
Leave-one-organ-out (LOOO) is substantially harder than the pooled setting of RQ1 because the target
anatomy is \emph{absent} from training. On the six-organ subset \morph{} attains the best mean PCC
($0.422$; Table~\ref{tab:looo_6organ}) and wins every held-out organ. On the full seven-organ HEST
panel it reaches $0.490$ (Appendix~Table~\ref{tab:looo_main}) versus $0.473$ for BLEEP, the strongest
baseline; BLEEP is stronger on IDC and SKCM there. Appendix~\ref{app:mouse} reports a complementary
seven-organ mouse experiment.
...
\caption{\textbf{Per-cohort cross-organ LOOO accuracy} (four of the six cohorts in
Table~\ref{tab:looo_6organ}; kidney and prostate omitted for readability, frozen UNI+CONCH).
One panel per held-out cohort; box = per-gene PCC over three seeds, $\Diamond$ = mean. \morph{}
(orange) leads on all four.}
\label{fig:box_looo5}
\end{figure}
...
\toprule
Cohort & ST-Net~\tiny{NBE'20} & Hist2ST~\tiny{BIB'22} & BLEEP~\tiny{NeurIPS'23} & STEM~\tiny{ICLR'25}$^\dagger$ & \morph{}+CONCH \\
\midrule
...
\begin{keybox}
\textbf{Key Finding (RQ3):} Local distance-biased attention is the primary driver of accuracy---
enabling it alone lifts mean PCC from $0.608$ to $0.640$, while global and slide context add little
without it---and the predictor is provably invariant to spot translation, rotation, and reflection.
\end{keybox}
...
(KMeans, $k{=}6$ domains) using either predicted or measured expression and compare the induced spatial
partitions with adjusted Rand index (ARI) and normalized mutual information (NMI) on held-out slides of
the six-organ subset (Table~\ref{tab:downstream5}). \morph{} leads on both ARI ($0.173$) and NMI
($0.242$), ahead of the strongest baseline on each metric (BLEEP at $0.147$ ARI, ST-Net at $0.201$ NMI).
The differences remain small and slide-level variation is large (std $\approx$ mean), so we caution
against over-interpreting them as evidence of uniformly improved domain recovery; the full seven-cohort
version is in Appendix~\ref{app:spatial}.
...
selection. MCToGene uses a different ten-cohort protocol and appears in Appendix~\ref{app:published}.
...
\section{Computation Resources and Inference Tools}
\label{app:compute}
...
a whole slide in a single forward pass---no iterative sampling.
...
\label{app:encoder}
We repeat the intra-cohort PCC@50 comparison (the 50-gene block of Table~\ref{tab:main}, which uses
the concatenated UNI\,+\,CONCH features) with each foundation encoder \emph{in isolation}:
UNI only (ViT-L/16, 1024-d; Table~\ref{tab:enc_uni}) and CONCH only (512-d; Table~\ref{tab:enc_conch}).
Everything else---models, cohorts, gene panels, splits, and training budget---is held fixed.
\textcolor{gray}{\textit{---}} entries are still training.

\begin{table}[H]
\centering
\caption{\textbf{Intra-cohort PCC@50 with \emph{UNI-only} features (1024-d).} Same six cohorts,
models, and protocol as Table~\ref{tab:main}; mean$_{\text{std}}$ over three seeds; best per row in
\best{bold}.}
\label{tab:enc_uni}
\setlength{\tabcolsep}{3pt}
\resizebox{\linewidth}{!}{%
\begin{tabular}{l ccccc}
\toprule
Cohort & ST-Net & Hist2ST & BLEEP & STEM & \morph{} (Ours) \\
\midrule
IDC~\tiny{Breast}    & \running & \running & \running & \running & \running \\
CCRCC~\tiny{Kidney}  & \running & \running & \running & \running & \running \\
LUNG~\tiny{Lung}     & \running & \running & \running & \running & \running \\
PAAD~\tiny{Pancreas} & \running & \running & \running & \running & \running \\
PRAD~\tiny{Prostate} & \running & \running & \running & \running & \running \\
SKCM~\tiny{Skin}     & \running & \running & \running & \running & \running \\
\midrule
\textbf{Average}     & \running & \running & \running & \running & \running \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Intra-cohort PCC@50 with \emph{CONCH-only} features (512-d).} Same six cohorts,
models, and protocol as Table~\ref{tab:main}; mean$_{\text{std}}$ over three seeds; best per row in
\best{bold}.}
\label{tab:enc_conch}
\setlength{\tabcolsep}{3pt}
...
\section{Model complexity and inference scaling}
...
\section{Per-Cohort Breakdown of the Error Metrics (Table~\ref{tab:error_strategy})}
\label{app:mse_mae_full}

Table~\ref{tab:error_strategy} reports MSE and MAE \emph{averaged over the six cohorts} for each
gene-selection strategy and panel size. Tables~\ref{tab:mse_strategy}--\ref{tab:mae_size} give the
per-cohort values behind those averages: the mean of each column here equals the corresponding entry
of Table~\ref{tab:error_strategy}. Same six cohorts, five models, frozen UNI\,+\,CONCH features,
splits, and training budget as the Pearson Tables~\ref{tab:panel_strategy} and~\ref{tab:main};
mean$_{\text{std}}$ over three seeds; best (lowest) per row in \best{bold}.

\begin{table}[H]
\centering
\caption{\textbf{Intra-cohort MSE $\downarrow$ across 50-gene selection strategies, per cohort.}}
\label{tab:mse_strategy}
\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{0.9}
\resizebox{\linewidth}{!}{%
\begin{tabular}{llccccc}
\toprule
Panel & Cohort & ST-Net & Hist2ST & BLEEP & STEM & \morph{} (Ours) \\
\midrule
\multirow{6}{*}{\textbf{HVG}} & IDC~\tiny{Breast} & $2.98{\pm}0.28$ & $2.86{\pm}0.39$ & $2.43{\pm}0.04$ & $2.80{\pm}0.06$ & \best{$2.24{\pm}0.03$} \\
 & CCRCC~\tiny{Kidney} & $0.59{\pm}0.01$ & $0.57{\pm}0.01$ & $0.60{\pm}0.01$ & $0.80{\pm}0.04$ & \best{$0.55{\pm}0.02$} \\
 & LUNG~\tiny{Lung} & $3.32{\pm}0.03$ & $1.80{\pm}0.06$ & \best{$1.62{\pm}0.04$} & $2.10{\pm}0.09$ & $1.67{\pm}0.04$ \\
 & PAAD~\tiny{Pancreas} & $2.04{\pm}0.27$ & $1.34{\pm}0.21$ & \best{$1.13{\pm}0.26$} & $1.71{\pm}0.21$ & $1.17{\pm}0.17$ \\
 & PRAD~\tiny{Prostate} & $0.63{\pm}0.03$ & $0.56{\pm}0.00$ & $0.57{\pm}0.02$ & $0.82{\pm}0.01$ & \best{$0.54{\pm}0.03$} \\
 & SKCM~\tiny{Skin} & $2.41{\pm}0.08$ & $2.24{\pm}0.08$ & $1.55{\pm}0.05$ & $2.25{\pm}0.17$ & \best{$1.23{\pm}0.03$} \\
\midrule
\multirow{6}{*}{\textbf{DEG}} & IDC~\tiny{Breast} & $5.42{\pm}0.32$ & $2.55{\pm}0.83$ & $1.93{\pm}0.24$ & $2.04{\pm}0.28$ & \best{$1.85{\pm}0.17$} \\
 & CCRCC~\tiny{Kidney} & $2.10{\pm}0.16$ & $1.58{\pm}0.01$ & $1.67{\pm}0.01$ & $1.90{\pm}0.06$ & \best{$1.58{\pm}0.10$} \\
 & LUNG~\tiny{Lung} & $3.87{\pm}0.04$ & $1.95{\pm}0.01$ & \best{$1.87{\pm}0.01$} & $2.45{\pm}0.05$ & $1.92{\pm}0.02$ \\
 & PAAD~\tiny{Pancreas} & $3.35{\pm}0.38$ & $1.46{\pm}0.24$ & \best{$1.32{\pm}0.29$} & $1.97{\pm}0.33$ & $1.34{\pm}0.24$ \\
 & PRAD~\tiny{Prostate} & $2.18{\pm}0.31$ & $1.55{\pm}0.03$ & $1.48{\pm}0.03$ & $2.01{\pm}0.06$ & \best{$1.34{\pm}0.03$} \\
 & SKCM~\tiny{Skin} & $3.09{\pm}0.09$ & $1.45{\pm}0.03$ & $1.08{\pm}0.10$ & $1.37{\pm}0.13$ & \best{$0.83{\pm}0.01$} \\
\midrule
\multirow{6}{*}{\textbf{HMHVG}} & IDC~\tiny{Breast} & $4.72{\pm}0.60$ & $2.24{\pm}0.46$ & $2.09{\pm}0.28$ & $2.28{\pm}0.13$ & \best{$1.80{\pm}0.15$} \\
 & CCRCC~\tiny{Kidney} & $1.46{\pm}0.04$ & $1.24{\pm}0.08$ & $1.23{\pm}0.02$ & $1.41{\pm}0.02$ & \best{$1.12{\pm}0.09$} \\
 & LUNG~\tiny{Lung} & $4.55{\pm}0.02$ & $1.89{\pm}0.02$ & $1.68{\pm}0.04$ & $2.05{\pm}0.00$ & \best{$1.60{\pm}0.01$} \\
 & PAAD~\tiny{Pancreas} & $2.58{\pm}0.26$ & $1.37{\pm}0.28$ & \best{$1.21{\pm}0.23$} & $1.76{\pm}0.22$ & $1.24{\pm}0.19$ \\
 & PRAD~\tiny{Prostate} & $1.52{\pm}0.15$ & \best{$1.35{\pm}0.02$} & $1.40{\pm}0.07$ & $1.89{\pm}0.15$ & $1.52{\pm}0.35$ \\
 & SKCM~\tiny{Skin} & $6.71{\pm}0.41$ & $2.90{\pm}0.91$ & $1.80{\pm}0.26$ & $2.47{\pm}0.64$ & \best{$1.10{\pm}0.03$} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Intra-cohort MSE $\downarrow$ across gene-panel sizes, per cohort.}}
\label{tab:mse_size}
\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{0.9}
\resizebox{\linewidth}{!}{%
\begin{tabular}{llccccc}
\toprule
Genes & Cohort & ST-Net & Hist2ST & BLEEP & STEM & \morph{} (Ours) \\
\midrule
\multirow{6}{*}{\textbf{10}} & IDC~\tiny{Breast} & $11.00{\pm}1.47$ & $5.23{\pm}0.58$ & $5.49{\pm}0.09$ & $5.66{\pm}0.05$ & \best{$5.02{\pm}0.49$} \\
 & CCRCC~\tiny{Kidney} & $1.05{\pm}0.01$ & \best{$0.90{\pm}0.03$} & $0.93{\pm}0.03$ & $1.11{\pm}0.04$ & $0.90{\pm}0.03$ \\
 & LUNG~\tiny{Lung} & $8.61{\pm}0.29$ & $4.13{\pm}0.09$ & $3.07{\pm}0.05$ & $3.53{\pm}0.29$ & \best{$2.93{\pm}0.07$} \\
 & PAAD~\tiny{Pancreas} & $6.85{\pm}1.05$ & $3.09{\pm}0.77$ & \best{$2.54{\pm}0.73$} & $3.14{\pm}0.80$ & $2.79{\pm}1.01$ \\
 & PRAD~\tiny{Prostate} & $1.57{\pm}0.23$ & $1.21{\pm}0.01$ & $1.25{\pm}0.03$ & $1.46{\pm}0.02$ & \best{$1.17{\pm}0.06$} \\
 & SKCM~\tiny{Skin} & $7.44{\pm}0.12$ & $5.23{\pm}0.34$ & $3.87{\pm}0.03$ & $4.18{\pm}0.29$ & \best{$2.91{\pm}0.16$} \\
\midrule
\multirow{6}{*}{\textbf{50}} & IDC~\tiny{Breast} & $2.98{\pm}0.28$ & $2.86{\pm}0.39$ & $2.43{\pm}0.04$ & $2.80{\pm}0.06$ & \best{$2.24{\pm}0.03$} \\
 & CCRCC~\tiny{Kidney} & $0.59{\pm}0.01$ & $0.57{\pm}0.01$ & $0.60{\pm}0.01$ & $0.80{\pm}0.04$ & \best{$0.55{\pm}0.02$} \\
 & LUNG~\tiny{Lung} & $3.32{\pm}0.03$ & $1.80{\pm}0.06$ & \best{$1.62{\pm}0.04$} & $2.10{\pm}0.09$ & $1.67{\pm}0.04$ \\
 & PAAD~\tiny{Pancreas} & $2.04{\pm}0.27$ & $1.34{\pm}0.21$ & \best{$1.13{\pm}0.26$} & $1.71{\pm}0.21$ & $1.17{\pm}0.17$ \\
 & PRAD~\tiny{Prostate} & $0.63{\pm}0.03$ & $0.56{\pm}0.00$ & $0.57{\pm}0.02$ & $0.82{\pm}0.01$ & \best{$0.54{\pm}0.03$} \\
 & SKCM~\tiny{Skin} & $2.41{\pm}0.08$ & $2.24{\pm}0.08$ & $1.55{\pm}0.05$ & $2.25{\pm}0.17$ & \best{$1.23{\pm}0.03$} \\
\midrule
\multirow{6}{*}{\textbf{100}} & IDC~\tiny{Breast} & $4.22{\pm}0.27$ & $2.33{\pm}0.24$ & $2.04{\pm}0.16$ & $2.47{\pm}0.29$ & \best{$2.02{\pm}0.04$} \\
 & CCRCC~\tiny{Kidney} & $1.55{\pm}0.07$ & $1.12{\pm}0.05$ & $1.22{\pm}0.01$ & $1.45{\pm}0.02$ & \best{$1.11{\pm}0.02$} \\
 & LUNG~\tiny{Lung} & $4.38{\pm}0.05$ & $1.79{\pm}0.04$ & \best{$1.38{\pm}0.00$} & $1.84{\pm}0.03$ & $1.47{\pm}0.02$ \\
 & PAAD~\tiny{Pancreas} & $1.79{\pm}0.23$ & $1.09{\pm}0.19$ & \best{$0.96{\pm}0.18$} & $1.47{\pm}0.14$ & $1.00{\pm}0.12$ \\
 & PRAD~\tiny{Prostate} & $1.58{\pm}0.17$ & $1.60{\pm}0.09$ & \best{$1.47{\pm}0.05$} & $1.80{\pm}0.07$ & $1.60{\pm}0.25$ \\
 & SKCM~\tiny{Skin} & $6.40{\pm}0.13$ & $3.13{\pm}0.36$ & $2.28{\pm}0.08$ & $2.36{\pm}0.03$ & \best{$1.55{\pm}0.04$} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Intra-cohort MAE $\downarrow$ across 50-gene selection strategies, per cohort.}}
\label{tab:mae_strategy}
\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{0.9}
\resizebox{\linewidth}{!}{%
\begin{tabular}{llccccc}
\toprule
Panel & Cohort & ST-Net & Hist2ST & BLEEP & STEM & \morph{} (Ours) \\
\midrule
\multirow{6}{*}{\textbf{HVG}} & IDC~\tiny{Breast} & $1.28{\pm}0.06$ & $1.27{\pm}0.09$ & $1.12{\pm}0.01$ & $1.21{\pm}0.02$ & \best{$1.10{\pm}0.01$} \\
 & CCRCC~\tiny{Kidney} & $0.58{\pm}0.00$ & $0.57{\pm}0.00$ & $0.58{\pm}0.00$ & $0.63{\pm}0.02$ & \best{$0.57{\pm}0.01$} \\
 & LUNG~\tiny{Lung} & $1.36{\pm}0.01$ & $1.01{\pm}0.02$ & \best{$0.95{\pm}0.01$} & $1.06{\pm}0.03$ & $0.96{\pm}0.01$ \\
 & PAAD~\tiny{Pancreas} & $1.01{\pm}0.06$ & $0.87{\pm}0.07$ & \best{$0.79{\pm}0.09$} & $0.94{\pm}0.05$ & $0.80{\pm}0.05$ \\
 & PRAD~\tiny{Prostate} & $0.57{\pm}0.02$ & $0.56{\pm}0.01$ & \best{$0.55{\pm}0.01$} & $0.62{\pm}0.01$ & $0.55{\pm}0.02$ \\
 & SKCM~\tiny{Skin} & $1.02{\pm}0.02$ & $1.06{\pm}0.02$ & $0.92{\pm}0.02$ & $1.06{\pm}0.04$ & \best{$0.79{\pm}0.00$} \\
\midrule
\multirow{6}{*}{\textbf{DEG}} & IDC~\tiny{Breast} & $2.02{\pm}0.07$ & $1.21{\pm}0.20$ & \best{$1.03{\pm}0.07$} & $1.07{\pm}0.08$ & $1.04{\pm}0.06$ \\
 & CCRCC~\tiny{Kidney} & $1.13{\pm}0.03$ & \best{$0.98{\pm}0.01$} & $1.00{\pm}0.00$ & $1.05{\pm}0.02$ & $0.99{\pm}0.04$ \\
 & LUNG~\tiny{Lung} & $1.57{\pm}0.01$ & $1.09{\pm}0.00$ & \best{$1.05{\pm}0.00$} & $1.19{\pm}0.01$ & $1.05{\pm}0.01$ \\
 & PAAD~\tiny{Pancreas} & $1.47{\pm}0.09$ & $0.96{\pm}0.08$ & \best{$0.88{\pm}0.10$} & $1.07{\pm}0.10$ & $0.90{\pm}0.08$ \\
 & PRAD~\tiny{Prostate} & $1.17{\pm}0.08$ & $0.96{\pm}0.01$ & $0.93{\pm}0.01$ & $1.07{\pm}0.02$ & \best{$0.89{\pm}0.02$} \\
 & SKCM~\tiny{Skin} & $1.40{\pm}0.02$ & $0.92{\pm}0.01$ & $0.78{\pm}0.03$ & $0.88{\pm}0.04$ & \best{$0.70{\pm}0.00$} \\
\midrule
\multirow{6}{*}{\textbf{HMHVG}} & IDC~\tiny{Breast} & $1.84{\pm}0.14$ & $1.14{\pm}0.13$ & $1.04{\pm}0.08$ & $1.11{\pm}0.05$ & \best{$1.00{\pm}0.06$} \\
 & CCRCC~\tiny{Kidney} & $0.95{\pm}0.01$ & $0.87{\pm}0.03$ & $0.85{\pm}0.01$ & $0.91{\pm}0.01$ & \best{$0.83{\pm}0.03$} \\
 & LUNG~\tiny{Lung} & $1.72{\pm}0.00$ & $1.08{\pm}0.01$ & $1.00{\pm}0.00$ & $1.09{\pm}0.00$ & \best{$0.98{\pm}0.00$} \\
 & PAAD~\tiny{Pancreas} & $1.21{\pm}0.05$ & $0.91{\pm}0.09$ & \best{$0.84{\pm}0.08$} & $0.99{\pm}0.06$ & $0.84{\pm}0.05$ \\
 & PRAD~\tiny{Prostate} & $0.92{\pm}0.04$ & \best{$0.88{\pm}0.01$} & $0.88{\pm}0.03$ & $1.00{\pm}0.05$ & $0.94{\pm}0.13$ \\
 & SKCM~\tiny{Skin} & $2.18{\pm}0.05$ & $1.38{\pm}0.29$ & $1.00{\pm}0.07$ & $1.19{\pm}0.17$ & \best{$0.81{\pm}0.01$} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Intra-cohort MAE $\downarrow$ across gene-panel sizes, per cohort.}}
\label{tab:mae_size}
\setlength{\tabcolsep}{3pt}\renewcommand{\arraystretch}{0.9}
\resizebox{\linewidth}{!}{%
\begin{tabular}{llccccc}
...
Genes & Cohort & ST-Net & Hist2ST & BLEEP & STEM & \morph{} (Ours) \\
\midrule
\multirow{6}{*}{\textbf{10}} & IDC~\tiny{Breast} & $2.87{\pm}0.21$ & $1.79{\pm}0.12$ & \best{$1.75{\pm}0.02$} & $1.83{\pm}0.04$ & $1.77{\pm}0.11$ \\
 & CCRCC~\tiny{Kidney} & $0.78{\pm}0.01$ & $0.74{\pm}0.01$ & $0.74{\pm}0.01$ & $0.79{\pm}0.01$ & \best{$0.74{\pm}0.02$} \\
 & LUNG~\tiny{Lung} & $2.55{\pm}0.05$ & $1.62{\pm}0.02$ & $1.34{\pm}0.02$ & $1.41{\pm}0.06$ & \best{$1.26{\pm}0.01$} \\
 & PAAD~\tiny{Pancreas} & $2.22{\pm}0.16$ & $1.41{\pm}0.21$ & \best{$1.25{\pm}0.21$} & $1.36{\pm}0.20$ & $1.33{\pm}0.28$ \\
 & PRAD~\tiny{Prostate} & $0.94{\pm}0.05$ & $0.83{\pm}0.01$ & \best{$0.82{\pm}0.01$} & $0.86{\pm}0.01$ & $0.83{\pm}0.02$ \\
 & SKCM~\tiny{Skin} & $2.09{\pm}0.02$ & $1.85{\pm}0.10$ & $1.56{\pm}0.00$ & $1.61{\pm}0.06$ & \best{$1.34{\pm}0.03$} \\
\midrule
\multirow{6}{*}{\textbf{50}} & IDC~\tiny{Breast} & $1.28{\pm}0.06$ & $1.27{\pm}0.09$ & $1.12{\pm}0.01$ & $1.21{\pm}0.02$ & \best{$1.10{\pm}0.01$} \\
 & CCRCC~\tiny{Kidney} & $0.58{\pm}0.00$ & $0.57{\pm}0.00$ & $0.58{\pm}0.00$ & $0.63{\pm}0.02$ & \best{$0.57{\pm}0.01$} \\
 & LUNG~\tiny{Lung} & $1.36{\pm}0.01$ & $1.01{\pm}0.02$ & \best{$0.95{\pm}0.01$} & $1.06{\pm}0.03$ & $0.96{\pm}0.01$ \\
 & PAAD~\tiny{Pancreas} & $1.01{\pm}0.06$ & $0.87{\pm}0.07$ & \best{$0.79{\pm}0.09$} & $0.94{\pm}0.05$ & $0.80{\pm}0.05$ \\
 & PRAD~\tiny{Prostate} & $0.57{\pm}0.02$ & $0.56{\pm}0.01$ & \best{$0.55{\pm}0.01$} & $0.62{\pm}0.01$ & $0.55{\pm}0.02$ \\
 & SKCM~\tiny{Skin} & $1.02{\pm}0.02$ & $1.06{\pm}0.02$ & $0.92{\pm}0.02$ & $1.06{\pm}0.04$ & \best{$0.79{\pm}0.00$} \\
\midrule
\multirow{6}{*}{\textbf{100}} & IDC~\tiny{Breast} & $1.66{\pm}0.05$ & $1.15{\pm}0.05$ & \best{$1.05{\pm}0.06$} & $1.16{\pm}0.08$ & $1.06{\pm}0.02$ \\
 & CCRCC~\tiny{Kidney} & $0.91{\pm}0.02$ & $0.80{\pm}0.02$ & $0.82{\pm}0.00$ & $0.88{\pm}0.01$ & \best{$0.80{\pm}0.00$} \\
 & LUNG~\tiny{Lung} & $1.71{\pm}0.01$ & $1.05{\pm}0.01$ & \best{$0.89{\pm}0.00$} & $1.01{\pm}0.01$ & $0.91{\pm}0.01$ \\
 & PAAD~\tiny{Pancreas} & $0.97{\pm}0.06$ & $0.80{\pm}0.07$ & \best{$0.74{\pm}0.06$} & $0.87{\pm}0.05$ & $0.75{\pm}0.04$ \\
 & PRAD~\tiny{Prostate} & $0.94{\pm}0.05$ & $0.91{\pm}0.02$ & \best{$0.87{\pm}0.01$} & $0.94{\pm}0.02$ & $0.92{\pm}0.09$ \\
 & SKCM~\tiny{Skin} & $1.93{\pm}0.02$ & $1.36{\pm}0.06$ & $1.13{\pm}0.01$ & $1.15{\pm}0.01$ & \best{$0.92{\pm}0.01$} \\
\bottomrule
\end{tabular}%
}
\end{table}
