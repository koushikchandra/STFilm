...
\begin{table}[t]
\centering
\caption{\textbf{Architectural components across representative histology to spatial transcriptomic (H\&E to ST) predictors.}
...
and MAE$\downarrow$ are mean$_{\text{std}}$ over three seeds; best per column in \best{bold}. The
``Full, random nbr.'' row replaces spatial-kNN local attention with random neighbours. The complete
model (L+G+S) is best on PCC and MSE; local attention is the dominant stream.}
\label{tab:ablation}
\small
\setlength{\tabcolsep}{5pt}
\resizebox{\linewidth}{!}{%
...
absolute agreement remains modest; the full six-cohort version appears in Appendix~\ref{app:spatial}.
...
\begin{table}[H]
\centering
\caption{\textbf{Case study: per-gene recovery of canonical cancer markers} (intra-cohort, held-out
slides; per-gene Pearson $\uparrow$, mean over folds and three seeds). \morph{} beats BLEEP on every
marker; best per row in \best{bold}.}
\label{tab:casestudy}
\small
\setlength{\tabcolsep}{6pt}
...
\begin{table}[H]
\centering
\caption{\textbf{Spatial-domain recovery (all six cohorts).} ARI/NMI between domains clustered
(KMeans, $k{=}6$) from predicted vs.\ measured expression on held-out slides of all six retained
HEST-1k cohorts; mean$_{\mathrm{std}}$ over three seeds, higher is better, best per column in
\best{bold}. Companion to the six-organ Table~\ref{tab:downstream5}.}
...
to the MSE objective; averaged over six organ groups, three seeds.}
\label{tab:lossdata}
\small
\setlength{\tabcolsep}{6pt}
\begin{tabular}{l l c}
