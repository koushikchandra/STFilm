...
(Table~\ref{tab:ablation}). The full model (L\,+\,G\,+\,S) is best (PCC $0.61$), and local attention is
the critical stream: every configuration that includes it reaches $\approx0.60$, whereas dropping local
in favor of global\,+\,slide (G\,+\,S) falls to $0.57$. Crucially, replacing the spatial $k$NN neighbors
with random ones lowers the full model from $0.61$ to $0.58$, showing that the benefit comes from
...
(the full L\,+\,G\,+\,S model scores $0.61$, every local-containing configuration reaches $\approx0.60$
while dropping local falls to $0.57$, and spatial $k$NN neighbors beat random ones), and the predictor
is provably invariant to spot translation, rotation, and reflection.
\end{keybox}
