...
\subsection{Dataset: HEST-1k Cohort Statistics}
...
\subsection{Computation Resources and Inference Tools}
\label{app:compute}
...
Several limitations temper our conclusions. \textbf{(i)}~The cross-organ advantage is not uniform: on
breast (IDC), BLEEP outperforms \morph{} under leave-one-organ-out transfer. \textbf{(ii)}~Two cohorts
(lung, skin) contain only two slides each, so their per-cohort estimates carry high variance.
\textbf{(iii)}~Leakage-safe cross-organ gene panels are small (7--14 genes), which caps the absolute
correlations attainable and limits the resolution of the spatial-domain-recovery analysis, whose
margins are small relative to slide-level variation. \textbf{(iv)}~The gap between pooled and
leave-one-organ-out performance shows that predicting an \emph{entirely unseen} organ remains an open
problem. Future work includes larger and imputation-based gene panels, categorical metadata
conditioning (organ/platform via FiLM/adaLN), and extending the evaluation to more organs and to
cross-species transfer.

\subsection{Model complexity and inference scaling}
...
\subsection{Mouse cross-organ diagnostic}
\label{app:mouse}
...
\subsection{Spatial-domain recovery}
\label{app:spatial}
\begin{table}[H]
\centering
...
\subsection{Full per-cohort results}
...
\subsection{Cross-organ LOOO transfer, expanded baselines (UNI\,+\,CONCH)}
...
\subsection{Cross-organ LOOO transfer with UNI features}
...
\subsection{Published Per-Cohort Comparison}
\label{app:published}
...
\subsection{Gene Panel Comparison}
\label{app:panel}
...
\clearpage
\subsection{Proofs of the formal guarantees}
\label{app:formal_guarantees}

\subsubsection{Proof of Proposition~\ref{prop:coordinate_invariance}}
...
\subsubsection{Proof of Proposition~\ref{prop:permutation_equivariance}}
...
\subsection{Three-Metric Intra-Cohort Summary}
\label{app:metrics}
...
\subsection{Per-Cohort MSE and MAE (Intra-Cohort)}
\label{app:mse_mae}
...
\subsection{Cross-Organ Generalization: PCC and MSE}
...
\subsection{Intra-Cohort Results with UNI\,+\,CONCH Features, Extended Baselines}
