...
\section{Per-Cohort MSE and MAE by Strategy and Panel Size}
\label{app:mse_mae_full}

Tables~\ref{tab:mse_strategy}--\ref{tab:mae_size} give the per-cohort MSE and MAE breakdowns that mirror the Pearson Tables~\ref{tab:panel_strategy} (gene-selection strategies) and~\ref{tab:main} (gene-panel sizes), and underlie the averaged error metrics of Table~\ref{tab:error_strategy}. Same six cohorts, five models, frozen UNI\,+\,CONCH features, splits, and training budget; mean$_{\text{std}}$ over three seeds; best (lowest) per row in \best{bold}.
