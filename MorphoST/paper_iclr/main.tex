...
Starting with the intra-cohort setting, Table~\ref{tab:main} reports the comparison across gene-panel
sizes. For PCC@50, \morph{} obtains the highest mean PCC ($0.496$) against $0.444$ for BLEEP, the
strongest baseline, an $11.6\%$ relative improvement, and it leads on all six cohorts; the margin is
widest on kidney and prostate ($+0.072$ and $+0.064$) and narrowest on lung ($+0.024$). Importantly,
this ranking is robust to panel size: under a smaller \textbf{10-gene} panel (PCC@10,
Table~\ref{tab:main_g10}), \morph{} again leads the average ($0.600$ vs.\ $0.545$ for BLEEP) and tops
five of six cohorts, with correlations rising across all methods because the retained genes are the
most variable, and hence the most predictable.
...
entirely absent from training. As expected, accuracy falls relative to the pooled setting of RQ1, yet
\morph{} still transfers best. On the six-organ subset it attains the highest mean PCC ($0.422$;
Table~\ref{tab:looo_6organ}) and wins every held-out organ, and on the full seven-organ HEST panel it
reaches $0.490$ versus $0.473$ for BLEEP, the strongest baseline (although BLEEP is stronger on IDC and
SKCM there). This shows that \morph{}'s inductive biases carry over to unseen anatomy, which supports
RQ2; a complementary seven-organ mouse experiment in Appendix~\ref{app:mouse} shows the same trend.
...
combinations of local (L), global (G), and slide-level (S) context on SKCM and LUNG. The picture is
clear: local attention provides the largest gain, since enabling it alone raises mean PCC from $0.608$
to $0.640$, whereas global and slide context add little without it; the complete model is nonetheless
best ($0.643$). This identifies distance-biased neighborhood aggregation as the primary contributor in
the intra-cohort setting, which supports the design rationale in RQ3 (claims under organ shift would
require a matched LOOO ablation, not reported here).
...
slides of the six-organ subset (Table~\ref{tab:downstream5}). Here \morph{} again leads, on both ARI
($0.173$) and NMI ($0.242$), ahead of the strongest baseline on each metric (BLEEP at $0.147$ ARI,
ST-Net at $0.201$ NMI). That said, the differences are small and slide-level variation is large
(std $\approx$ mean), so we caution against reading them as uniformly improved domain recovery; the full
seven-cohort version appears in Appendix~\ref{app:spatial}.
