...
\begin{abstract}
Spatial transcriptomics reveals how gene expression is organized across a tissue, but it is costly and slow, motivating methods that predict it directly from routine H\&E histology. Existing predictors, however, are typically trained per dataset, are sensitive to the arbitrary coordinate frame of each slide, or rely on slow iterative generative sampling. We introduce \morph{}, a compact spatial transformer that predicts a slide's spatial gene expression in a single forward pass by fusing three complementary views of context in every layer: local attention over spatial nearest neighbors, global self-attention across all spots, and an attention-pooled slide representation. Because spot coordinates enter only through pairwise distances, its predictions are provably invariant to translations, rotations, and reflections of the coordinate frame when image features are fixed. On six human organ cohorts from HEST-1k, evaluated under intra-cohort, pooled multi-organ, and leave-one-organ-out settings, \morph{} attains the highest mean Pearson correlation (PCC) on every cohort, improving over the strongest baseline by $11.6\%$ (50 genes) and $11.4\%$ (100 genes), and the lowest MSE and MAE; the gains are statistically significant and persist across UNI, CONCH, and combined features. \morph{} also preserves spatial tissue structure, recovering the expression domains of the measured data more faithfully than the strongest baseline on $78$--$90\%$ of held-out slides. Beyond predictive performance,

\morph{} recovers canonical cancer-marker genes, reproducing the spatial expression of melanoma (mean PCC $0.90$) and breast HER2/ER/PR markers, so it reads out clinically actionable molecular signals from H\&E alone. It generalizes to held-out organs ($+6.0\%$ over the closest baseline BLEEP), while transfer to entirely unseen anatomy remains the main open challenge.
\end{abstract}
