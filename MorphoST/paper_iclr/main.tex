...
The exact coordinate guarantee assumes consistent resolution of $k$NN distance ties. On held-out
CCRCC and PRAD slides, rigid coordinate transforms preserve aggregate PCC to within $0.000166$, although
individual predictions can change.
\end{abstract}
...
\noindent Our experiments address five research questions:
...
\textbf{RQ3 (Ablation):} Which context pathways and spatial-neighborhood choices drive predictive accuracy?
\textbf{RQ4 (Coordinate invariance):} How stable are predictions and accuracy under transformations of the spot-coordinate frame?
\textbf{RQ5 (Structure):} How well do predicted expression profiles preserve spatial domains identified from measured expression, as measured by ARI and NMI?
...
under every tested strategy and panel size (Table~\ref{tab:main}).
...
BLEEP on $87\%$ of genes ($n{=}300$; Wilcoxon $p<10^{-30}$). A summary of the paired statistical
tests is reported in
...
\centering
\setcounter{table}{0}
\caption{\textbf{Intra-cohort HEST-1k: 50-gene selection strategies.} Dataset PCC is mean $\pm$ SD over three seeds; final columns are six-cohort mean PCC, MSE, and MAE. Best per strategy in \best{bold}; $\uparrow$ higher and $\downarrow$ lower is better.}
...
\centering
\setcounter{table}{1}
\caption{\textbf{Intra-cohort HEST-1k: gene-panel sizes.} Dataset PCC is mean $\pm$ SD over three seeds; final columns are six-cohort mean PCC, MSE, and MAE. Best per panel size in \best{bold}; $\uparrow$ higher and $\downarrow$ lower is better.}
\label{tab:main}
\label{tab:main_g10}
\label{tab:main_g100}
\setlength{\tabcolsep}{3pt}
...
\midrule
\multirow{5}{*}{\textbf{PCC@50}} & ST-Net & $0.551{\pm}0.01$ & $0.163{\pm}0.02$ & $0.518{\pm}0.01$ & $0.366{\pm}0.03$ & $0.294{\pm}0.02$ & $0.665{\pm}0.01$ & $0.425$ & $1.995$ & $0.969$ \\
 & Hist2ST & $0.309{\pm}0.07$ & $0.184{\pm}0.02$ & $0.470{\pm}0.01$ & $0.248{\pm}0.04$ & $0.333{\pm}0.02$ & $0.522{\pm}0.03$ & $0.335$ & $1.561$ & $0.890$ \\
 & BLEEP & $0.519{\pm}0.00$ & $0.175{\pm}0.02$ & $0.562{\pm}0.01$ & $0.412{\pm}0.03$ & $0.348{\pm}0.01$ & $0.645{\pm}0.01$ & $0.444$ & $1.318$ & $0.819$ \\
 & STEM & $0.439{\pm}0.01$ & $0.123{\pm}0.00$ & $0.423{\pm}0.03$ & $0.233{\pm}0.02$ & $0.207{\pm}0.02$ & $0.491{\pm}0.04$ & $0.310$ & $1.747$ & $0.920$ \\
 & \morph{} (Ours) & \best{$0.581{\pm}0.01$} & \best{$0.252{\pm}0.01$} & \best{$0.589{\pm}0.01$} & \best{$0.437{\pm}0.03$} & \best{$0.410{\pm}0.01$} & \best{$0.707{\pm}0.01$} & \best{$0.496$} & \best{$1.233$} & \best{$0.794$} \\
\midrule
...
\label{sec:transfer}
Having established strong performance on organs represented during training, we next
evaluate transfer to an entirely unseen organ using leave-one-organ-out (LOOO)
evaluation. Each fold withholds one organ from training and tests on that organ,
ensuring that its anatomy is absent from the training set.

As shown in Table~\ref{tab:looo_6organ}, \morph{} achieves the highest PCC on all six
held-out organs and obtains the best mean PCC overall ($0.422$), ahead of BLEEP
($0.398$). This improvement is statistically significant (Wilcoxon
$p{=}0.018$; Appendix~\ref{app:significance}). Although performance decreases
relative to the pooled setting ($0.599$), the consistent per-organ gains indicate
that \morph{} transfers more effectively to unseen anatomy. A complementary mouse
experiment in Appendix~\ref{app:mouse} shows the same trend.

\begin{table}[H]
\centering
\caption{\textbf{Cross-organ LOOO transfer (six-organ subset).} Dataset columns report PCC as mean $\pm$ standard deviation over three seeds; each fold holds out one organ (leakage-safe panels, frozen UNI\,+\,CONCH). The final three columns average PCC, MSE, and MAE equally across the six held-out organs. Best per column in \best{bold}; $\uparrow$ higher is better and $\downarrow$ lower is better.}
...
BLEEP & $0.401{\pm}0.04$ & $0.058{\pm}0.00$ & $0.630{\pm}0.00$ & $0.497{\pm}0.03$ & $0.091{\pm}0.00$ & $0.708{\pm}0.01$ & $0.398$ & $1.410$ & \best{$0.847$} \\
STEM$^\dagger$ & \running & \running & \running & \running & \running & \running & \running & \running & \running \\
\midrule
...
{\footnotesize $^\dagger$STEM values are being recomputed with the correct $T{=}1000$ DDPM schedule (an earlier run used $T{=}50$); results pending.}
\end{table}
...
\label{sec:design}
Given \morph{}'s accuracy, we next ask \emph{which} components drive it. To isolate each context stream, we run a factorial study over all eight
...
aggregation as the primary contributor. Encoder-feature ablations are reported separately in
Appendix~\ref{app:encoder}.

\begin{table}[H]
\centering
\caption{\textbf{Context-stream ablation} on two datasets: six-organ POOLED (top) and SKCM intra-cohort (bottom), both with frozen UNI\,+\,CONCH. Local (L), global (G), slide (S) streams; the ``Full, random nbr.'' row uses random rather than spatial-$k$NN neighbours. Mean$_{\text{std}}$ over three seeds; best per column \emph{within each block} in \best{bold}.}
\label{tab:ablation}
\small
...
\toprule
Dataset & Configuration & Streams & PCC $\uparrow$ & MSE $\downarrow$ & MAE $\downarrow$ \\
\midrule
\multirow{4}{*}{\rotatebox[origin=c]{90}{\shortstack{Six-organ\\POOLED}}}
...
\midrule
\multirow{4}{*}{\rotatebox[origin=c]{90}{\shortstack{SKCM\\(intra)}}}
 & Global\,+\,Local  & L\,+\,G       & $0.704{\pm}0.006$ & $1.240{\pm}0.029$ & $0.804{\pm}0.012$ \\
 & Global\,+\,Slide  & G\,+\,S       & $0.659{\pm}0.003$ & $1.411{\pm}0.069$ & $0.855{\pm}0.023$ \\
 & Full, random nbr. & L$_{\text{rand}}$\,+\,G\,+\,S & $0.671{\pm}0.004$ & $1.408{\pm}0.014$ & $0.859{\pm}0.007$ \\
 & \morph{} (ours)   & L\,+\,G\,+\,S & \best{$0.709{\pm}0.002$} & \best{$1.240{\pm}0.041$} & \best{$0.798{\pm}0.018$} \\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{table}[H]
\centering
\caption{\textbf{Real-slide coordinate-transform check.} The Table~\ref{tab:panel_strategy} CCRCC and PRAD UNI\,+\,CONCH HVG models (six and two held-out folds) and the Table~\ref{tab:pooled6} six-organ pooled UNI\,+\,CONCH model (five held-out folds; shared 14-gene panel) are evaluated across three trained seeds per setting, with image features fixed. Coordinates are translated by $(37.2,-91.4)$, rotated by $0.731$ radians, reflected across the vertical axis, or randomly permuted. PCC is averaged across folds, then reported as mean $\pm$ standard deviation across seeds. Each $\Delta$PCC is relative to the original coordinates within the same setting; scores across gene panels are not directly comparable.}
...
We test coordinate transformations on held-out CCRCC and PRAD slides from the
HVG experiments in Table~\ref{tab:panel_strategy}, and on the six-organ pooled slides from
Table~\ref{tab:pooled6}. The original scores reproduce the reported results: $0.252{\pm}0.01$
for CCRCC, $0.410{\pm}0.01$ for PRAD, and $0.599$ for the pooled setting. Across six CCRCC,
two PRAD, and five pooled folds, with three trained seeds per setting, the three-seed mean PCC
changes by at most $0.000166$ after translation, rotation, reflection, or spot permutation
(Table~\ref{tab:invariance}); the corresponding mean MSE and MAE change by at most $0.000128$
and $0.000043$. The pooled model uses its own shared 14-gene panel, not the intra-cohort HVG panel.
Reflection leaves predictions exactly unchanged in this test. Translation, rotation, and permutation
can change the $k$NN selection when distances tie or are numerically close: although aggregate
accuracy is stable, individual predictions are not exactly invariant on these real slides.
...
Per-gene PCC@50 & \morph{} vs. BLEEP & $n=300$ genes & \morph{} wins 87\%; $p<10^{-30}$ \\
LOOO PCC & \morph{} vs. BLEEP & six held-out organs & $p=0.018$ \\
Spatial ARI & \morph{} vs. BLEEP & $n=58$ slides & \morph{} wins 78\%; $p<0.001$ \\
Spatial NMI & \morph{} vs. BLEEP & $n=58$ slides & \morph{} wins 90\%; $p<0.001$ \\
\bottomrule
\end{tabular}
\end{table}
